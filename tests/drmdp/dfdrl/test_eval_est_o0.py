...
class _TermEnv(gym.Env):
    ...
    def __init__(self, obs_dim: int = 2, act_dim: int = 1, steps_to_term: int = 3):
        super().__init__()
        self.observation_space = gym.spaces.Box(
            low=-1.0, high=1.0, shape=(obs_dim,), dtype=np.float32
        )
...
class TestEvaluateInteractive:
    def test_runs_single_episode_and_prints_mae(self, capsys):
        model = est_o0.RNetwork(state_dim=2, action_dim=1)
        model.eval()
        env = _TermEnv(obs_dim=2, act_dim=1, steps_to_term=3)
        eval_est_o0.evaluate_interactive(model, env, num_episodes=1)
        captured = capsys.readouterr()
        assert "MAE" in captured.out

    def test_runs_multiple_episodes(self, capsys):
        model = est_o0.RNetwork(state_dim=2, action_dim=1)
        model.eval()
        env = _TermEnv(obs_dim=2, act_dim=1, steps_to_term=2)
        eval_est_o0.evaluate_interactive(model, env, num_episodes=2)
        captured = capsys.readouterr()
...
class TestMain:
    ...
    def test_interactive_mode(self, tmp_path, capsys):
        ...
        model = est_o0.RNetwork(state_dim=state_dim, action_dim=action_dim)
        torch.save(model.state_dict(), str(tmp_path / "model_o0.pt"))
        env = _TermEnv(obs_dim=state_dim, act_dim=action_dim, steps_to_term=3)
        with patch(
            "sys.argv",
            [
                "prog",
